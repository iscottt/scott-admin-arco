...
const transformAuthRouteToVueRoute = (item: any) => {
  ...
  itemRoute.component = item.parentId == 0 ? layout : getViewComponent(itemRoute.name || 'not-found-page');
  // 注意：单独路由没有children
  if (!item.children) {
    // 捕获无效路由的需特殊处理
    if (item.funcUrl === '/not-found-page') {
      ...
      const parentRoute: any = {
        path: parentPath,
        // component: layout,
        component: item.parentId == 0 ? layout : getViewComponent(itemRoute.name),
        name: item.funcUrl.replace('/', '').replace(new RegExp('/', 'g'), '_'),
        meta: {
          locale: item.funcName,
          icon: 'icon-file',
...
const transformAuthRouteToMenu = (routes: any[]): any[] => {
  ...
  routes.forEach((route) => {
    const routeName = route.funcUrl.replace('/', '').replace(new RegExp('/', 'g'), '_');
